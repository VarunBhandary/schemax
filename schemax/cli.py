...
@cli.command()
@click.option('--schema-file', '-f', required=True, type=click.Path(exists=True), 
...
@click.pass_context
def generate(ctx, schema_file: str, target_catalog: str, target_schema: Optional[str], 
    ...
    try:
        ...
        with Progress(
            ...
            if verbose:
                console.print(f"[dim]Found {len(schema_def.schemas)} schema(s) in definition[/dim]")
            ...
            current_state = databricks_client.get_current_state(target_catalog, target_schema)
...
@click.pass_context
def apply(ctx, schema_file: str, target_catalog: str, target_schema: Optional[str], auto_approve: bool):
    """Apply schema changes to target environment."""
    try:
        # First generate the changes
        ctx.invoke(generate, schema_file=schema_file, target_catalog=target_catalog, 
                  target_schema=target_schema, dry_run=True)
        ...
        console.print("[dim]This will execute the generated SQL script against the target environment.[/dim]")
