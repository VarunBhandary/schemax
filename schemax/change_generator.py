...
class SchemaAnalyzer(dspy.Signature):
    ...
    desired_schema = dspy.InputField(desc="YAML schema definition describing desired state")
    current_state = dspy.InputField(desc="JSON description of current Databricks environment state") 
    catalog_name = dspy.InputField(desc="Target catalog name")
    schema_name = dspy.InputField(desc="Target schema name (optional)")
    
    analysis = dspy.OutputField(desc="Detailed analysis of differences between desired and current state")
    changes_needed = dspy.OutputField(desc="List of specific changes required to reach desired state")
...
class SQLGenerator(dspy.Signature):
    ...
    sql_script = dspy.OutputField(desc="Complete SQL DDL script to implement all changes")
    warnings = dspy.OutputField(desc="Any warnings or considerations for applying the changes")
...
class ChangeValidator(dspy.Signature):
    ...
    validation_issues = dspy.OutputField(desc="Any validation issues or improvements needed")
...
class SchemaChangeAnalyzer(dspy.Module):
    ...
    def forward(self, desired_schema: str, current_state: str, catalog_name: str, schema_name: str = ""):
        return self.analyze(
...
class SQLScriptGenerator(dspy.Module):
    ...
    def forward(self, analysis: str, changes_needed: str, catalog_name: str, schema_name: str):
...
class ChangeGenerator:
    ...
    def generate_changes(self, schema_def: SchemaDefinition, current_state: CurrentState) -> ChangeScript:
        ...
        try:
            ...
            if hasattr(validation_result, 'is_valid') and validation_result.is_valid.lower() == 'false':
                if hasattr(validation_result, 'corrected_sql') and validation_result.corrected_sql:
                    final_sql = validation_result.corrected_sql
                if hasattr(validation_result, 'validation_issues'):
                    warnings.append(f"SQL validation issues: {validation_result.validation_issues}")
        ...
        for schema in schema_def.schemas:
            ...
            for table in schema.tables:
                ...
                for column in table.columns:
                    ...
                    result.append(f"    Column: {column.name} {column.type} {nullable_str}")
                    if column.comment:
        ...
        for schema_name, schema_info in current_state.schemas.items():
            ...
            for table_name, table_info in schema_tables.items():
                ...
                for column in columns:
                    ...
                    col_type = column.get('type_text', column.get('type_name', 'UNKNOWN'))
                    result.append(f"    Column: {column['name']} {col_type} {nullable_str}")
    ...
    def _extract_changes_list(self, changes_needed: str) -> List[str]:
        ...
        return changes if changes else ["Schema changes needed (see SQL script for details)"] 