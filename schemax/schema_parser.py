...
class SchemaParser:
    ...
    def _parse_column(self, column_data: Dict[str, Any]) -> Column:
        ...
        if 'type' not in column_data:
            raise ValidationError(f"Column '{column_data['name']}' must have a 'type' field")
        ...
        for schema in schema_def.schemas:
            ...
            for table in schema.tables:
                ...
                if len(column_names) != len(set(column_names)):
                    errors.append(f"Duplicate column names in table '{schema.name}.{table.name}'")
                
                # Validate external table has location
                if table.type == "EXTERNAL" and not table.location:
                    errors.append(f"External table '{schema.name}.{table.name}' must specify a location")
